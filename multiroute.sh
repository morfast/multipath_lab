...
do
    ...
    PPPGATE=$(ip route | grep 'ppp0.*src' | awk '{print $1}')
    ROUTECMD="${ROUTECMD}nexthop via ${PPPGATE} dev ppp${i}  weight 2 \\
              "
    ...
    iptables -t nat -A POSTROUTING -o ppp${i} -j SNAT --to ${IP_PPP}
...
ip route add $(ip route show table main | grep 'eth0.*src 59') table E0
ip route add default via ${LOCALGATE} table E0

ip rule add prio 20000 from ${LOCALIP} table E0
iptables -D INPUT -i eth0 -j ACCEPT
iptables -A INPUT -i eth0 -j ACCEPT
iptables -D FORWARD -i eth0 -j ACCEPT
iptables -A FORWARD -i eth0 -j ACCEPT

#ROUTECMD="${ROUTECMD} nexthop via ${LOCALGATE} dev eth0  weight 10 "
...
ip route add 222.47.62.142 via ${PPPGATE}
ip route add 222.47.29.93 via ${PPPGATE}


ip route del default
eval "${ROUTECMD}"
#ip route add default via ${PPPGATE}
